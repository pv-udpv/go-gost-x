...
type JA4Fingerprint struct {
	...
	// ALPN: First 2 characters of first ALPN protocol, "00" if none
	ALPN string
...
func GenerateJA4(data *JA4Data) (*JA4Fingerprint, error) {
	...
	// ALPN: First 2 characters of first ALPN protocol, "00" if none
	if len(data.ALPNProtocols) > 0 && len(data.ALPNProtocols[0]) >= 2 {
		fp.ALPN = data.ALPNProtocols[0][:2]
	} else if len(data.ALPNProtocols) > 0 && len(data.ALPNProtocols[0]) == 1 {
		fp.ALPN = data.ALPNProtocols[0] + "0"
	} else {
		fp.ALPN = "00"
	}
...
// NOTE: This function has been moved to the fingerprint package to handle cross-package dependencies
// It will be re-implemented during the full integration phase
...
// NOTE: Functions that depend on ja3 package types (GetJA4FromClientHelloFile, etc.)
// have been temporarily removed during refactoring and will be re-implemented
// in the fingerprint package to handle cross-package dependencies properly.
