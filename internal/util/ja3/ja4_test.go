...
func TestConvertJA3ToJA4(t *testing.T) {
	ja3Data := &JA3Data{
		Version:      0x0303,
	...
	ja4Data := ConvertJA3ToJA4(ja3Data, "example.com", false)

	if ja4Data.TLSVersion != 0x0303 {
		t.Errorf("TLSVersion = %x, want %x", ja4Data.TLSVersion, 0x0303)
	}
	if ja4Data.ServerName != "example.com" {
		t.Errorf("ServerName = %s, want example.com", ja4Data.ServerName)
	}
	if ja4Data.IsQUIC {
		t.Error("IsQUIC should be false")
	}
	if len(ja4Data.CipherSuites) != 2 {
		t.Errorf("CipherSuites count = %d, want 2", len(ja4Data.CipherSuites))
	}
}
